...
return {
  ...
  opts = function()
    local dashboard_sections = {
      ...
      { section = "keys", gap = 1, keys = {
        { icon = " ", key = "n", desc = "New File", action = function() if _G.schedule_close_explorer_next_file then _G.schedule_close_explorer_next_file() end vim.cmd("ene") end },
        ...
        { icon = "󰱼 ", key = "f", desc = "Find Files", action = function() if _G.schedule_close_explorer_next_file then _G.schedule_close_explorer_next_file() end local ok, p = pcall(require, "snacks.picker"); if ok then p.files() end end },
        { icon = " ", key = "s", desc = "Search", action = function() if _G.schedule_close_explorer_next_file then _G.schedule_close_explorer_next_file() end local ok, p = pcall(require, "snacks.picker"); if ok then p.grep() end end },
    ...
    return {
      ...
      dashboard = { enabled = true, sections = dashboard_sections },
    }
  end,
  config = function(_, opts)
    ...
    vim.api.nvim_create_autocmd("LspProgress", {
      callback = function(ev)
        ...
        pcall(vim.notify, msg, "info", { id = "lsp_progress", title = "LSP Progress", icon = icon, replace = true })
      end,
    })
  end,
}
